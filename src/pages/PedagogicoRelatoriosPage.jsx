...
export default function PedagogicoRelatoriosPage() {
  ...
  async function loadData() {
    try {
      setLoading(true)
      const [reports, classRows, teacherRows] = await Promise.all([
        schoolCrudService.listClassReports({ ...filters, archived: false }),
      ...
      setError(err.message ?? 'Não foi possível carregar relatórios de aula.')
    } finally {
      setLoading(false)
    }
  }
  ...
  async function saveReport() {
    try {
      await schoolCrudService.createClassReport(form)
      setModalOpen(false)
      setForm(emptyForm)
      await loadData()
    } catch (err) {
      setError(err.message ?? 'Não foi possível salvar relatório de aula.')
    }
  }

  async function archiveReport(id) {
    await schoolCrudService.archiveClassReport(id)
    await loadData()
  }

  async function deleteReport(id) {
    if (!window.confirm('Excluir relatório de aula?')) return
    await schoolCrudService.deleteClassReport(id)
    await loadData()
  }
  ...
  return (
    <AppShell
      title="Pedagógico: Relatórios de Aula"
      subtitle="Cada relatório mantém somente metadados e link externo (Google Docs)."
    >
      ...
      <section className="mb-4 grid gap-3 rounded-2xl border border-sky-100 bg-white p-3 md:grid-cols-4">
        <input
          ...
          onChange={(event) => setFilters((prev) => ({ ...prev, report_date: event.target.value }))}
        />
      ...
      {loading ? <LoadingRow text="Carregando relatórios de aula..." /> : null}
              ...
              <tbody className="divide-y divide-slate-100">
                {paginatedRows.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50/60">
                    <td className="px-4 py-3 font-semibold text-slate-900">{row.class_name}</td>
                    <td className="px-4 py-3 text-slate-700">{row.teacher_name}</td>
                    <td className="px-4 py-3 text-slate-700">{row.report_date}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button type="button" className="rounded-lg border border-slate-200 px-2 py-1 text-xs" onClick={() => archiveReport(row.id)}>
                          <span className="inline-flex items-center gap-1"><Archive className="h-3.5 w-3.5" /> Arquivar</span>
                        </button>
                        <button type="button" className="rounded-lg border border-rose-200 px-2 py-1 text-xs text-rose-700" onClick={() => deleteReport(row.id)}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
            <div className="border-b border-slate-100 px-5 py-3">
              <h3 className="text-lg font-semibold">Adicionar relatório de aula</h3>
            </div>
            <div className="grid gap-3 p-5 md:grid-cols-2">
              ...
              <label className="text-sm text-slate-700">
                ...
                <input
                  ...
                  onChange={(event) => setForm((prev) => ({ ...prev, report_date: event.target.value }))}
                />
              </label>
